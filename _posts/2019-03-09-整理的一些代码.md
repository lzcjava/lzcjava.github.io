...
.clipper-container .clipper-part {
  position: absolute;
  top: 0;
  bottom: 0;
  left: 0;
...
.clipper-container .clipper-part .pCanvas-container {
  position: absolute;
  top: 50%;
  left: 50%;
...
.clipper-container .mask.opacity {
  opacity: 0.8;
}
.clipper-container .gesture-mask {
  position: absolute;
  top: 0;
  bottom: 0;
  left: 0;
...
<image-clipper ref="clipper" v-if="visible" @ok='ok' @cancel='cancel' :img="imgUrl" :clipper-img-width="800" :clipper-img-height="800"></image-clipper>


import imageClipper from '@/components/VimageClipper.vue';
...
ok (data) {
  var that = this
  var dataURL = this.$refs.clipper.getBase64(data);
}
...
<transition name="eject">
    <div class="agreement" v-show="showAgreement">
        <h4 class="agreement-title">嘻嘻嘻嘻嘻</h4>
        <ol class="bold">
            <li>
                喜喜喜喜喜喜
            </li>
        </ol>
        <div class="agreement_bottom course-flex">
            <div class="course-flex-item agreement_cancel">取消</div>
            <div class="course-flex-item agreement_determine">确定</div>
        </div>
    </div>
</transition>
...
.eject-enter-active,.eject-leave-active {
  transition: all .4s ease;
}
.eject-enter,.eject-leave-to {
